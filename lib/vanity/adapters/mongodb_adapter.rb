module Vanity
  module Adapters
    ...
    class MongodbAdapter < AbstractAdapter
      ...
      def ab_add_conversion(experiment, alternative, identity, count = 1, extra_id = nil, implicit = false)
        if implicit
          @participants.update({ :experiment=>experiment, :identity=>identity }, { "$set"=>{ :seen=>alternative } }, :upsert=>true)
        else
        ...
        if extra_id.nil?
          ...
          @participants.update({ :experiment=>experiment, :identity=>identity }, { "$set"=>{ :converted=>alternative, :extra_id=>extra_id} }, :upsert=>true) if implicit || participating
        end  
